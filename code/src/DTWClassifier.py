...
class DTWWordClassifier:
    ...
    def get_DTW_dist_sample_to_class(self, sample_index, filter_by_handwriting=Utils.BLOCK_LETTER):
        if filter_by_handwriting in self.classes[sample_index]:
            sample_dst_file_path = Utils.RES_FOLDER_PATH + Utils.RES_DTW_DISTANCES +'_'+self.dataset_name+ self.time_stamp_last_execution+'\\'+self.considered_time_series_name+'\\'+str(sample_index)
            print('\nSTART TO FIND DISTANCES WITH CLASSES FOR THE ', sample_index, ' SAMPLE')
            total_class_set = set(self.classes)
            class_set = set()
            sample = self.considered_time_series[sample_index]
            ...
            outF.close()
                ...
                # min_distances[c] = min
            # print ('\nDISTANCES FOR THE ', sample_index, ' SAMPLE: ', correct_class, '\nAVG_DISTANCES: ', avg_distances, '\nMIN_DISTANCES: ', min_distances)
        print('\n', sample_index, ' HAS NOT A VALID HANDWRITING TYPE')
        return 0
    ...
    def make_DTW_distances_tables(self, temporary_series, temporary_series_names):
        ...
        for time_series in temporary_series:
            ...
            self.set_considered_time_series(time_series)
...
if __name__ == '__main__':
    ...
    a.make_DTW_distances_tables([a.filter_time_series_by_x_y(a.get_samples('movementPoints'))], ['movementPoints_filtered_by_x_y'])