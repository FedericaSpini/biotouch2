...
class AnonymousDataManager:
    ...
    def _load_dataframes(self, update):
        if not update and AnonymousDataManager._check_saved_pickles(self.dataset_name):
            self._read_pickles()
        else:
    ...
    def _shift(self):
        ...
        for l in Utils.INITIAL_POINTS_SERIES_TYPE:
            for dir in [Utils.X, Utils.Y, Utils.XY]:
                self.data_frames[Utils.GET_SHIFTED_POINTS_NAME(dir, l)] = self.data_frames[l].groupby(
                    Utils.WORD_ID).apply(f[dir])
        chrono.end()
